...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Dropdown navigation ── */
  const dropdownTriggers = document.querySelectorAll('.navbar__nav [data-dropdown]');
  ...
  dropdownTriggers.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.navbar__nav li.open').forEach(el => el.classList.remove('open'));
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          const active = document.querySelector(`.sub-nav a[href="#${entry.target.id}"]`);
  ...
  const fadeEls = document.querySelectorAll('.card, .timeline-item, .content-block');
